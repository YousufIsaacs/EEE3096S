...
#define EPOCH_2022 1640988000 //EPOCH time for Saturday, January 1, 2022 12:00:00 AM GMT+02:00
/* USER CODE END PD */
...
void pause_sec(float x)
{
	...
	//YOUR CODE HERE
	for (int i=0; i< x; i++ ); //Where x is DELAY1*DELAY2
}
...
uint8_t decToBcd(int val)
{
	...
	uint8_t Digit1 = val / 10 ; //Separates the tens
	uint8_t Digit2 = val % 10 ; //Separates the units

	uint8_t Bcd_number = (Digit1 << 4) + (Digit2); // Combines the two digits
...
int bcdToDec(uint8_t val)
{
	...
	int Digit1 = val >> 4; //Clears the right 4 bits

	uint8_t Digit_2 = val << 4 ; //Clears the left 4 bits
	int Digit2 = Digit_2 >> 4;   //Converts back to original number

	int Dec_number = Digit1 * 10 + Digit2; //Adds the 2 digits
...
int epochFromTime(TIME time){
	...
	//YOUR CODE HERE
	//Convert time values to int values
	...
	//Number of days that have past during past months
	int No_of_Days[12] = {0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334}; //NB. if we are in Jan it is only the No of days that have past


	int day = (year-22)*365 + (No_of_Days[month-1]) + (dayofmonth-1); //Get the number of days
	hour = hour + day*24;           //Get the number of hours
	minutes = minutes + hour*60;    //Get the number of minute
	seconds = seconds + minutes*60; //Get the number of seconds
